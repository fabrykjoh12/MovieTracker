...
- `VITE_CATALOG_API_URL`: public URL of the deployed catalog Worker
...
GitHub Actions reads the three public `VITE_` values from Actions variables. Repository variables are preferred; variables scoped to the `github-pages` environment are also supported. Pages requires both Neon URLs. The catalog URL is configured as a repository variable; if it is removed, the interface truthfully falls back to a visibly unavailable search state.
