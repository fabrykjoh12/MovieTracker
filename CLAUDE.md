...
- `TMDB_READ_ACCESS_TOKEN`: private provider credential used only by the local sync script or GitHub build runner
...
Real artwork for the curated catalog is generated by `npm run catalog:sync`. The script reads the stable mapping in `catalog/tmdb-seeds.json`, requests TMDB configuration and title details, and writes browser-safe metadata to `src/generated/tmdbCatalog.ts`. GitHub Actions runs it only when the private `TMDB_READ_ACCESS_TOKEN` Actions secret exists. The checked-in generated module is an empty safe fallback, so unconfigured builds continue to use explicit Unsplash demonstration imagery. TMDB attribution appears on the Profile page only when generated TMDB data is present.
