...
- **Fictional data presented as real, remaining findings.** Home's "This week" agenda (a hardcoded upcoming episode, availability notice, and Watch Together prompt) and "From friends" activity feed (fabricated Sara/Maya recommendations and reactions) are still hardcoded — friend relationships and activity don't exist yet in this beta at all (per this file's "Current state" section), so there is no real data to wire these to yet; the honest fix is likely to remove or clearly label them as a coming-soon preview rather than invent a friends system here. An earlier audit in this session additionally flagged Friends, Profile, and the Library calendar view for the same kind of seeded/demo content; those need the same file-by-file confirmation pass applied to the Shell and Home findings above before fixing, rather than a blanket rewrite.
