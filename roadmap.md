...
- **Fictional data presented as real.** Several screens still show hard-coded or seeded content next to a real authenticated user's data with no visual or semantic distinction, which risks a user believing it's theirs. Confirmed directly in this pass: `src/components/Shell.tsx`'s sidebar renders a fixed "4h 32m watched" stat and a fixed seven-bar sparkline (`[35, 80, 20, 56, 92, 44, 10]`) on every page, for every account, regardless of actual watch history. An earlier audit in this session additionally flagged Home, Friends, Profile, and the Library calendar view as showing seeded/demo social activity, taste stats, or calendar entries that are not derived from the signed-in user's real data — those need a file-by-file confirmation pass before fixing, the same rigor applied to the Shell finding above, rather than a blanket rewrite.
