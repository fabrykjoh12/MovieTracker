...
- **Fabricated identity, stats, and taste analysis on Profile.** `src/pages/Profile.tsx` displayed a fixed fake identity ("Alex Kim", "Oslo · watching thoughtfully since 2018"), fixed fake profile stats (486 watched / 74 this year / 1,032h / 18 rewatches), a fabricated "Defining favourite" (Arrival, with an invented quote, "Rewatched twice", and an editorial framing not tied to any real note), a "Taste DNA" card with five fixed bipolar-axis percentages presented as inferred from "143 confident verdicts" that were never counted, a "2026 so far" year-in-review card with fixed counts, a hardcoded four-title "Personal canon" (`["arrival","dune-part-two","dark","poor-things"]`) regardless of whether the account had ever verdicted any of them, and a fixed static "taste cloud" of genre/mood words — all shown identically for every account. Fixed by computing everything that's honestly derivable from the account's real data, and replacing what isn't with an honest state: `libraryOverview` (`src/domain.ts`) computes real watched/this-year/hours/rewatch counts from actual `WatchEvent`s and `UserMediaState`; `topFavourites` selects a real "defining favourite" and "personal canon" from titles that actually have a recorded Verdict, ordered by explicit rank then Verdict strength, with an honest empty state when none exist yet; `tasteCloud` builds a real genre/mood word cloud (sized by real frequency) from the account's own non-dropped library, also with an honest empty state. Taste DNA has no honest real substitute yet — CLAUDE.md's roadmap already defers "Advanced Taste DNA analytics" past this beta, and its five axes don't cleanly derive from existing Verdict/media fields — so that card became a "Coming soon" note instead of an invented inference. Covered by `src/domain.test.ts` and `src/pages/Profile.test.tsx`.
- **Fabricated dates, a fake reference "now," and hardcoded archive targets on Library.** `src/pages/Library.tsx`'s Calendar view showed a fixed 14-day range starting from a hardcoded day number with a hardcoded "today" index, a fake "New episode" notification, and a fake "Friday film" Watch Together prompt with a fake 3-person badge — none tied to a real date or real data. Separately, the "watchlist health" badge showed a fixed score of 82 with the fixed label "Focused and useful," and the "stale titles" clean-up prompt was hardcoded to "Three titles" and archived two literal, unconditional media IDs (`aftersun`, `portrait`) regardless of which titles were actually stale for the account — a real functional bug, not just display, since clicking "Review forgotten saves" archived the wrong titles (or titles the account didn't even have) instead of its real overdue watchlist items. A separate fixed reference date (`demoNow`, pinned to 2026-07-17) was also used to compute which titles counted as stale, silently drifting further from correct as real time passed. Fixed: the calendar now generates a real 14-day range around the real current date, marks the real today, and shows a real "Watched" badge (with a real poster/title link) on any day with an actual `WatchEvent`, instead of fabricated future content. The health badge shows the account's real title count. The clean-up prompt now reads its real count from the same `staleTitles` list it archives, is hidden entirely when nothing is actually stale, and archives exactly those real titles. `demoNow` was replaced with the real current time. Covered by `src/pages/Library.redesign.test.tsx`.

### P1 — confirmed, not yet fixed

- No further specific fictional-data findings are open from this pass's audit. A general "no fabricated data as real" review of the remaining pages (Discover, MediaDetail, Account) has not been performed and may still surface findings.
