...
- **Fictional data presented as real, remaining findings.** An earlier audit in this session flagged Friends, Profile, and the Library calendar view for the same kind of seeded/demo content as the Home and Shell findings above. Those need the same file-by-file confirmation pass applied here before fixing, rather than a blanket rewrite.
