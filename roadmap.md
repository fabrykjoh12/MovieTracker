...
- **Fictional data presented as real, remaining pages.** The Shell sidebar stat above is fixed; an earlier audit in this session additionally flagged Home, Friends, Profile, and the Library calendar view as showing seeded/demo social activity, taste stats, or calendar entries that are not derived from the signed-in user's real data. Those need the same file-by-file confirmation pass applied to the Shell finding before fixing, rather than a blanket rewrite.
